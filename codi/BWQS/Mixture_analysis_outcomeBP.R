...
vars<-c(
  ...
  #Outcome
  "hs2_zdia_bp.v3_2017_Time1", "hs2_zdia_bp.v3_2017_Time2",
  "hs2_zsys_bp.v3_2017_Time1", "hs2_zsys_bp.v3_2017_Time2"
  )
...
comps <- c(  "hs2_zdia_bp.v3_2017_Time1", "hs2_zdia_bp.v3_2017_Time2",
             "hs2_zsys_bp.v3_2017_Time1", "hs2_zsys_bp.v3_2017_Time2" )
...
for (x in names(data_list)){
  ...
  for (comp in comps){
    ...
    saveRDS(fit_lasso_mixt0_zbmi8, file = paste0("./results/ExWAS/Mixture_analysis_BP/fit_",result_name,".rds"))
    ...
    write.csv2(results_chemp_zbmi8, paste0("./results/ExWAS/Mixture_analysis_BP/res_",result_name,"1.csv"))
    write.csv2(data, paste0("./results/ExWAS/Mixture_analysis_BP/data/",result_name,"1.csv"))
    write_rds(data_reg, paste0("./results/ExWAS/Mixture_analysis_BP/data/data_reg_",result_name,"1.RDS"))
    
  }
}

#Forest plot--------------------------------------------------------------------
res_all_diat1 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_all_hs2_zdia_bp.v3_2017_Time11.csv") %>% mutate(Population = "Pooled")
res_all_syst1 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_all_hs2_zsys_bp.v3_2017_Time11.csv")%>% mutate(Population = "Pooled")
res_all_diat2 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_all_hs2_zdia_bp.v3_2017_Time21.csv")%>% mutate(Population = "Pooled")
res_all_syst2 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_all_hs2_zsys_bp.v3_2017_Time21.csv")%>% mutate(Population = "Pooled")

res_n_diat1 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_n_hs2_zdia_bp.v3_2017_Time11.csv") %>% mutate(Population = "North")
res_n_syst1 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_n_hs2_zsys_bp.v3_2017_Time11.csv")%>% mutate(Population = "North")
res_n_diat2 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_n_hs2_zdia_bp.v3_2017_Time21.csv")%>% mutate(Population = "North")
res_n_syst2 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_n_hs2_zsys_bp.v3_2017_Time21.csv")%>% mutate(Population = "North")

res_s_diat1 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_s_hs2_zdia_bp.v3_2017_Time11.csv") %>% mutate(Population = "South")
res_s_syst1 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_s_hs2_zsys_bp.v3_2017_Time11.csv")%>% mutate(Population = "South")
res_s_diat2 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_s_hs2_zdia_bp.v3_2017_Time21.csv")%>% mutate(Population = "South")
res_s_syst2 <- read.csv2("./results/ExWAS/Mixture_analysis_BP/res_data_s_hs2_zsys_bp.v3_2017_Time21.csv")%>% mutate(Population = "South")

df_diat1 <- rbind(res_all_diat1, res_n_diat1, res_s_diat1)
df_diat2 <- rbind(res_all_diat2, res_n_diat2, res_s_diat2)
df_syst1 <- rbind(res_all_syst1, res_n_syst1, res_s_syst1)
df_syst2 <- rbind(res_all_syst2, res_n_syst2, res_s_syst2)

df_diat1$Population <- factor(df_diat1$Population, 
                              levels =  c("North", "South", "Pooled"),
                              labels = c("North", "South", "Pooled"))
df_diat2$Population <- factor(df_diat2$Population, 
                              levels =  c("North", "South", "Pooled"),
                              labels = c("North", "South", "Pooled"))
df_syst1$Population <- factor(df_syst1$Population, 
                              levels =  c("North", "South", "Pooled"),
                              labels = c("North", "South", "Pooled"))
...
df_diat1$X[df_diat1$X == "Lifestyle factors"] <- "Maternal personal factors"
df_diat2$X[df_diat2$X == "Lifestyle factors"] <- "Maternal personal factors"
df_syst1$X[df_syst1$X == "Lifestyle factors"] <- "Maternal personal factors"
...
df_diat1$Label[df_diat1$Label == "" |is.na(df_diat1$Label)] <- df_diat1$X[df_diat1$Label == "" |is.na(df_diat1$Label)]
df_diat2$Label[df_diat2$Label == "" |is.na(df_diat2$Label)] <- df_diat2$X[df_diat2$Label == "" |is.na(df_diat2$Label)]

df_syst1$Label[df_syst1$Label == "" |is.na(df_syst1$Label)] <- df_syst1$X[df_syst1$Label == "" |is.na(df_syst1$Label)]
df_syst2$Label[df_syst2$Label == "" |is.na(df_syst2$Label)] <- df_syst2$X[df_syst2$Label == "" |is.na(df_syst2$Label)]


write_xlsx(list("DBP t1"=df_diat1,
                "DBP t2"=df_diat2,
                "SBP t1"=df_syst1,
                "SBP t2"=df_syst2),
           "./results/ExWAS/Mixture_analysis_BP/bwqs_results.xlsx")
...
dbpt1 <- ggplot(df_diat1[df_diat1$X %in% c("Air pollution", "Built environment", "Traffic", "Green spaces and walkability", "Metals", "OCs", "PFAS", "Maternal personal factors", "Diet", "Meteorological factors"),],
       aes(y = X, x = mean, xmin = `X2.5.`, xmax = `X97.5.`, shape = Population, colour = Population)) +
  geom_pointrange(size = 0.8, position = position_dodge(width = 0.5)) +
  scale_color_manual(values = colors) + 
...
# Function to prepare dataset for table
...
all_results <- bind_rows(
  prep_table(df_diat1, mix_name_5, "Metals (DBP in childhood)"),
  prep_table(df_syst2, mix_name_8, "Maternal personal factors (SBP in adolescence)"),
  prep_table(df_syst2, mix_name_9, "Diet (SBP in adolescence)")
)


# Show combined table
knitr::kable(all_results, digits = 3, align = "c",
...
saveWorkbook(wb, "Mixture_results_all.xlsx", overwrite = TRUE)


saveWorkbook(wb, "./results/ExWAS/Mixture_analysis_BP/Mixture_results_table.xlsx", overwrite = TRUE)
