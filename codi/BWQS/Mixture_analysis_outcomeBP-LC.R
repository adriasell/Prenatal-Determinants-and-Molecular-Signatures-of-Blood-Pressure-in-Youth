...
df_all<-cbind(proj_all, exposome_all, phenotype_all, "LC_BP"=LC_BP_all)
df_n<-cbind(proj_north, exposome_n, phenotype_n, "LC_BP"=LC_BP_n)
df_s<-cbind(proj_south, exposome_s, phenotype_s, "LC_BP"=LC_BP_s)
...
for (x in names(data_list)){
  ...
  for (comp in comps){
    ...
    if (x=="data_s"){
      formula = as.formula(paste0(comp, "~ h_cohort_INMA + h_parity_None + h_native_None + h_edumc_None + h_age_None")) # write only the name of the covariates
    }
    ...
    saveRDS(fit_lasso_mixt0_zbmi8, file = paste0("./results/ExWAS/Mixture_analysis_BP-LC/fit_",result_name,".rds"))
    ...
    write.csv2(results_chemp_zbmi8, paste0("./results/ExWAS/Mixture_analysis_BP-LC/res_",result_name,"1.csv"))
    write.csv2(data, paste0("./results/ExWAS/Mixture_analysis_BP-LC/data/",result_name,"1.csv"))
    write_rds(data_reg, paste0("./results/ExWAS/Mixture_analysis_BP-LC/data/data_reg_",result_name,"1.RDS"))
    
  }
}

#Forest plot--------------------------------------------------------------------
res_all <- read.csv2("./results/ExWAS/Mixture_analysis_BP-LC/res_data_all_LC_BP1.csv") %>% mutate(Population = "Pooled")
res_n <- read.csv2("./results/ExWAS/Mixture_analysis_BP-LC/res_data_n_LC_BP1.csv")%>% mutate(Population = "North")
res_s <- read.csv2("./results/ExWAS/Mixture_analysis_BP-LC/res_data_s_LC_BP1.csv") %>% mutate(Population = "South")


df_res <- rbind(res_all, res_n, res_s)


df_res$Population <- factor(df_res$Population, 
...
df_res <- df_res %>% left_join(codebook %>% select(Variable_name_TRANS, Label), by = c("X" = "Variable_name_TRANS"))



df_res$Label[df_res$Label == "" |is.na(df_res$Label)] <- df_res$X[df_res$Label == "" | is.na(df_res$Label)]


write_xlsx(list("BP LC"=df_res),
           "./results/ExWAS/Mixture_analysis_BP-LC/bwqs_results.xlsx")

#Plot
colors <- c("North" = "#070600",
            "South" = "#BCB8B1",  
            "Pooled" = "#8A817C") 

pdf("./results/ExWAS/Forest_plot/Mixture_analysis_BP_LC.pdf")
ggplot(df_res[df_res$X %in% c("Air pollution", "Built environment", "Traffic", "Green spaces and walkability", "Metals", "OCs", "PFAS", "Maternal personal factors", "Demographic factors", "Diet", "Meteorological factors"),],
  ...
  labs(y = "Exposure", x = "Beta (CI 95%)", title = "Mixture analysis Beta exposure-BP component") +
